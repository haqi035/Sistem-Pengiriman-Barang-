...
<div class="card p-6">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <a href="{{route('customer.orders.create')}}" class="btn-primary flex items-center gap-2"><i class="fas fa-plus"></i><span>Order Baru</span></a>
    </div>
    <div class="space-y-4">
        @forelse($orders as $o)
            ...
            <div class="flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 mb-3">
                        <a href="{{route('customer.orders.show',$o)}}" class="text-base font-bold text-blue-600 hover:text-blue-800">{{$o->resi_number}}</a>
                        <span class="badge-{{$o->current_status}} px-2.5 py-0.5 rounded-full text-xs font-semibold">{{$o->status_label}}</span>
                        <span class="text-xs text-slate-400 capitalize bg-slate-50 px-2 py-0.5 rounded-lg">{{$o->service_type}}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        <i class="fas fa-map-marker-alt text-blue-300"></i>
                        <span>{{$o->sender_city}}</span>
                        <i class="fas fa-arrow-right text-blue-200 text-xs"></i>
                        <span class="font-medium">{{$o->receiver_city}}</span>
                    </div>
                    <p class="text-xs text-slate-400 mt-1">{{$o->receiver_name}} • {{$o->created_at->format('d M Y')}}</p>
                </div>
                <div class="text-right flex-shrink-0">
                    <p class="text-base font-bold text-blue-700">Rp {{number_format($o->total_cost,0,',','.')}}</p>
                    <p class="text-xs text-slate-400">{{$o->weight}} kg</p>
                    <a href="{{route('customer.orders.show',$o)}}" class="mt-2 btn-secondary py-1.5 px-3 text-xs inline-block">Detail</a>
                </div>
            </div>
        </div>
        @empty
        <div class="py-16 text-center text-slate-400">
            ...
            <a href="{{route('customer.orders.create')}}" class="mt-3 btn-primary inline-flex items-center gap-2">
