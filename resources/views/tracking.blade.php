...
<div class="max-w-2xl mx-auto px-4 py-12">
    <div class="text-center mb-10">
        <a href="{{route('login')}}" class="inline-flex items-center gap-2 mb-6 font-bold text-xl text-blue-700">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-blue-50 p-8">
        <form method="GET" action="{{route('tracking')}}" class="flex gap-3">
            <div class="flex-1 relative">
                <i class="fas fa-barcode absolute left-3 top-1/2 -translate-y-1/2 text-blue-300"></i>
                <input type="text" name="resi" value="{{request('resi')}}" placeholder="Contoh: SHP-20240101-0001"
        ...
        <div class="mt-8 border-t border-blue-50 pt-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-lg font-bold text-blue-900">{{$order->resi_number}}</p>
                </div>
                <span class="badge-{{$order->current_status}} px-3 py-1.5 rounded-full text-xs font-semibold">
                    {{$order->status_label}}
                </span>
            </div>
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="bg-blue-50 rounded-xl p-4">
                    ...
                    <p class="text-sm font-semibold text-slate-800">{{$order->sender_name}}</p>
                    <p class="text-xs text-slate-500">{{$order->sender_city}}</p>
                </div>
                <div class="bg-blue-50 rounded-xl p-4">
                    <p class="text-xs text-slate-500 mb-1 font-medium">Penerima</p>
                    <p class="text-sm font-semibold text-slate-800">{{$order->receiver_name}}</p>
                    <p class="text-xs text-slate-500">{{$order->receiver_city}}</p>
                </div>
            </div>
            <div class="space-y-1">
                ...
                <div class="flex gap-4 pb-4">
                    <div class="flex flex-col items-center">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 {{$i===0?'bg-blue-500':'bg-blue-100'}}">
                            <i class="fas {{$i===0?'fa-check text-white':'fa-circle text-blue-300'}} text-xs"></i>
                        </div>
                        @if(!$loop->last)<div class="w-0.5 flex-1 bg-blue-100 mt-1"></div>@endif
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-slate-800">{{ucfirst(str_replace('_',' ',$status->status))}}</p>
                        <p class="text-xs text-slate-500">{{$status->description}}</p>
                        @if($status->location)<p class="text-xs text-blue-400 mt-0.5"><i class="fas fa-map-marker-alt mr-1"></i>{{$status->location}}</p>@endif
                        <p class="text-xs text-slate-400 mt-0.5">{{$status->created_at->format('d M Y, H:i')}} WIB</p>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="text-center mt-6">
        <a href="{{route('login')}}" class="text-sm text-blue-500 hover:text-blue-700 font-medium">
