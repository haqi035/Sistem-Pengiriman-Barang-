...
<div class="card p-6">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <a href="{{route('admin.customers.create')}}" class="btn-primary flex items-center gap-2">
                    ...
                    <td class="py-3.5 pr-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center flex-shrink-0">
                                <span class="text-white text-xs font-bold">{{strtoupper(substr($c->name,0,2))}}</span>
                            </div>
                            <span class="text-sm font-medium text-slate-700">{{$c->name}}</span>
                        </div>
                    </td>
                    <td class="py-3.5 pr-4 text-sm text-slate-600">{{$c->email}}</td>
                    <td class="py-3.5 pr-4 text-sm text-slate-600">{{$c->phone??'-'}}</td>
                    <td class="py-3.5 pr-4 text-sm font-semibold text-blue-600">{{$c->orders_count}} order</td>
                    <td class="py-3.5 pr-4">
                        <span class="{{$c->is_active?'bg-green-100 text-green-600':'bg-red-100 text-red-500'}} px-2.5 py-1 rounded-full text-xs font-semibold">
                            {{$c->is_active?'Aktif':'Nonaktif'}}
                        </span>
                    </td>
                    <td class="py-3.5">
                        <div class="flex items-center gap-2">
                            <a href="{{route('admin.customers.edit',$c)}}" class="text-blue-500 hover:text-blue-700 text-sm"><i class="fas fa-edit"></i></a>
                            <form method="POST" action="{{route('admin.customers.destroy',$c)}}" onsubmit="return confirm('Hapus pelanggan ini?')">
                                @csrf @method('DELETE')
